...
class FunctionUtils
{
    ...
    // \DB::raw("IF (group_image IS NOT NULL, " . \DB::raw("CONCAT('" . FunctionUtils::getS3Url(config('global.UPLOAD_PATHS.GROUP')) . "', group_image)") . ', NULL) AS group_image'),
    // FunctionUtils::getS3FileUrl(config('global.UPLOAD_PATHS.USER') . $createdBy->profile_image)
}
