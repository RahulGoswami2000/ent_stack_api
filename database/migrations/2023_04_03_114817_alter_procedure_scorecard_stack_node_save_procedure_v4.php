    ...
    public function up()
    {
        ...
        IF SSDATA IS NOT NULL AND JSON_UNQUOTE (JSON_EXTRACT (SSDATA, '$.nodes')) != ' ' OR JSON_UNQUOTE (JSON_EXTRACT (SSDATA, '$.nodes')) IS NOT NULL THEN
		    ...
		    END IF;				
                    ...
                    SET auto_assign_color = IF(JSON_UNQUOTE(JSON_EXTRACT(node_dt,'$.auto_assign_color')) IS NULL, 0,1);
                    SET node_data =  IF(item_arr_element IS NULL, 'NULL',CONCAT(\"'\",item_arr_element,\"'\"));
                    
                    IF node_data_type = 'metricBox' THEN
                    
                        SET @sqlsssave = CONCAT('INSERT INTO `scorecard_stack_nodes` (`id`, `scorecard_stack_id`, `node_id`, `node_data`, `auto_assign_color`, `assigned_to`, `goal_achieve_in_number`, `reminder`, `created_by`, `updated_by`, `deleted_by`, `deleted_at`, `created_at`, `updated_at`)
                        VALUES (NULL,',ssid,',',node_id,',',node_data,',',auto_assign_color,',',assigned_to,',',goal_achieve_in_number,',',reminder,',',created_by_resp,',NULL,NULL,NULL,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)');
